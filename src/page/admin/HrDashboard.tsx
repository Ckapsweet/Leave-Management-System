  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50" style={{ fontFamily: "'DM Sans', 'Noto Sans Thai', sans-serif" }}>
      ...
      <DashboardHeader
        user={user}
        tabs={[
          { key: "requests", label: "รายการลา", badge: pending },
          { key: "employees", label: "พนักงานทั้งหมด" },
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-gray-700">
                  รายการลา <span className="ml-2 text-gray-400 font-normal">({filtered.length} รายการ)</span>
                </h2>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {filtered.map((request) => {
                        ...
                        const avatar = avatarColor(request.user?.department);
                        const isHourly = request.leave_unit === "hour";
                        return (
                          <tr key={request.id} className="hover:bg-slate-50/70 cursor-pointer transition-colors" onClick={() => setSelected(request)}>
                            <td className="px-5 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-medium text-gray-800 whitespace-nowrap">{request.user?.full_name}</p>
                            ...
                            <td className="px-5 py-4">
                              <div className="flex flex-col gap-1">
                                <span className={`px-2.5 py-1 rounded-full text-xs font-medium w-fit ${typeColor}`}>{request.leave_type.name}</span>
                                {isHourly && (
                            ...
                            <td className="px-5 py-4">
                              ...
                              {!isHourly && request.start_date !== request.end_date && <p className="text-xs text-gray-400">ถึง {fmtDate(request.end_date)}</p>}
                              {isHourly && request.start_time && <p className="text-xs text-gray-400">{request.start_time} - {request.end_time} น.</p>}
                            </td>
                            ...
                            <td className="px-5 py-4 text-sm text-gray-500 max-w-[180px] truncate">{request.reason}</td>
                            <td className="px-5 py-4">
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {filteredEmployees.map((employee) => {
                        const avatar = avatarColor(employee.department);
                        ...
                        return (
                          <tr key={employee.id} className="hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => handleEmployeeClick(employee)}>
                            <td className="px-5 py-4">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${avatar}`}>
                                ...
                                <div>
                                  <p className="text-sm font-medium text-gray-800 whitespace-nowrap">{employee.full_name}</p>
                            ...
                            <td className="px-5 py-4" onClick={(event) => event.stopPropagation()}>
                              <div className="flex items-center gap-2">
                                {canEditBalance && (
                                  <button
                                    onClick={() => openBalanceModal({ id: employee.id, full_name: employee.full_name, employee_code: employee.employee_code, department: employee.department })}
...
type HrTab = "requests" | "employees" | "reports";
