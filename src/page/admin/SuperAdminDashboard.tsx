...
export default function AdminDashboard() {
  ...
  const canEditAnyEnglishName = (user?.role === "manager" || user?.role === "admin") && filteredEmployees.length > 0;
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-slate-50 border-b border-gray-100 text-left">
                        ...
                        {(canEditAnyEmployeeBalance || canEditAnyEnglishName) && <th className="px-5 py-3 text-xs font-semibold text-gray-400"></th>}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {filteredEmployees.map((emp) => {
                        ...
                        return (
                          <tr key={emp.id} className="hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => handleEmployeeClick(emp)}>
                            ...
                            {(canEditAnyEmployeeBalance || canEditAnyEnglishName) && <td className="px-5 py-4" onClick={(e) => e.stopPropagation()}>
