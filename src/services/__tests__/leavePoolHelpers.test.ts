...
describe("deriveLeavePoolFromRequests", () => {
  it("uses exact total_hours for hourly leave instead of rounded stored days", () => {
    ...
    expect(result?.used_days).toBe(3.5 / 7.5);
    expect(result?.remaining).toBe(3 - 3.5 / 7.5);
    expect(result?.balances?.[0].used_days).toBe(3.5 / 7.5);
    expect(result?.balances?.[0].remaining).toBe(3 - 3.5 / 7.5);
  });
});
